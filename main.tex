...
\midrule
1 & \texttt{observe\_signals()} and update telemetry confidence for contention, queueing, freshness, and retention pressure. \\
2 & \texttt{identify\_state\_objects()} whose boundary is currently active: hot operator shards, KV pages, retrieval indexes, or replay buffers. \\
3 & \texttt{check\_hard\_constraints()} for SLA, memory limit, recovery envelope, and quality floor before any optimization action. \\
4 & \texttt{rank\_actions()} over admit, place, migrate, refresh, compact, and evict using expected utility and maintenance debt. \\
5 & \texttt{apply\_bounded\_action()} with explicit rollback or retry semantics when telemetry is delayed or conflicting. \\
6 & \texttt{account\_outcome()} by charging the decision against tail latency, quality drift, and long-horizon reuse value. \\
\bottomrule
\end{tabularx}
\end{table*}
...
The earliest seam was explicit transition semantics. Aurora and Borealis treated operator graphs and adaptation paths as runtime-managed artifacts, while MillWheel, Naiad, and Dataflow elevated event time, pending work, watermarks, and partial-order progress into the control plane rather than leaving them as hidden bookkeeping~\cite{abadi2003aurora,abadi2005borealis,akidau2013millwheel,murray2013naiad,akidau2015dataflow}. This shift matters because it separates two questions that older runtimes often conflated: whether an update is logically visible, and whether the corresponding state is physically stable enough to migrate, checkpoint, or replay.

A second precursor seam is progress-aware state advancement. Trill, Differential Dataflow, Flink, and StreamCloud each made a different part of that seam visible: incremental maintenance state, frontier or timestamp progression, and elastic ownership transfer all determine whether state can be updated continuously without turning coordination into a hidden bottleneck~\cite{chandramouli2014trill,murray2013differential,carbone2015flink,gulisano2012streamcloud}. This family matters because it frames access scheduling not only as conflict management, but also as safe progress over partially materialized state.
...
These topology decisions are not independent from fault handling. They define where barriers accumulate, which partitions replay together, and how much skew migration must absorb during reconfiguration. In that sense, access topology already encodes a latent recovery policy even before the runtime exposes one explicitly~\cite{zhang2019briskstream,zhang2020concurrent,carbone2015asynchronous,zhao2024recovery,mao2025spacker}.

A second mature seam is recovery coupling. Recovery is no longer a pure failure-mode concern; it is a steady-state design partner. Asynchronous snapshotting showed early that checkpoint alignment is not passive bookkeeping: the runtime must coordinate barriers, in-flight state, and snapshot boundaries so that fault tolerance remains lightweight during normal operation~\cite{carbone2015asynchronous}. Recent recovery work extends that logic to multicore transactional streaming, where replay scheduling and recovery metadata must be co-designed with the steady-state execution plan~\cite{zhao2024recovery}. Checkpoint cadence, log granularity, and migration mechanism therefore jointly define whether latency remains bounded during faults and reconfiguration. Systems that isolate these concerns administratively often pay duplicated state-transfer overhead.
...
One mature seam is orchestration and predictability. Clipper externalized batching, caching, and model-choice policy, while Clockwork turned execution-time predictability into a dispatchable control signal for request isolation and SLO management~\cite{crankshaw2017clipper,gujarati2020clockwork}. A second seam is memory layout and allocation policy. Paging-style approaches and explicit sharing surfaces improved memory efficiency and therefore expanded feasible batching regions~\cite{kwon2023pagedattention}. A third seam is phase-aware control: separating prefill and decode characteristics exposes different bottlenecks and enables targeted scheduling and placement actions~\cite{agrawal2024sarathi,zhong2024distserve,patel2024splitwise}. A fourth seam is control-plane and tenant-aware governance: Nexus made cluster occupancy and batching a first-class scheduling surface, InferLine and INFaaS showed that queue state, provisioning decisions, and heterogeneous replica choice already form a serving control loop around latency objectives, FlexGen made offload placement part of the memory lifecycle, and Punica plus AlpaServe showed that adapter state and partitioned model state introduce additional multiplexed memory layers whose isolation and sharing policies materially change utilization and interference~\cite{shen2019nexus,crankshaw2020inferline,romero2021infaas,sheng2023flexgen,chen2024punica,li2023alpaserve}.

The harder systems case appears when these seams interact at cluster scale. DistServe, Splitwise, and FastGen show that separating prefill from decode is useful precisely because the two phases stress different resources, but Punica and AlpaServe make clear that the state being routed is not just generic KV memory: it often carries tenant-specific adapters, partition metadata, and multiplexing constraints whose residency rules change interference patterns~\cite{zhong2024distserve,patel2024splitwise,holmes2024fastgen,chen2024punica,li2023alpaserve}. Read together with Nexus, InferLine, and INFaaS, the design implication is that short-lived serving state already lives under a cluster-level governance loop of occupancy signals, queue backpressure, provisioning decisions, and admission control~\cite{shen2019nexus,crankshaw2020inferline,romero2021infaas}. Future runtimes should therefore treat phase splitting and tenant multiplexing as a joint optimization problem: they must decide where prefill state is created, which decode path inherits it, and when adapter or model-partition placement should override raw batching efficiency to preserve tail latency and isolation.
...
One mature mechanism family is external-memory orchestration. REALM, RAG, RETRO, and Atlas differ in training setup and scale, but all treat the indexed corpus as a reusable state layer whose refresh path and lookup boundary materially influence downstream quality~\cite{guu2020realm,lewis2020rag,borgeaud2022retro,izacard2022atlas}. A second family is index-structure control. Dense dual-encoder systems improved retrieval quality and throughput tradeoffs, while late-interaction approaches preserved richer matching signals with manageable online cost by shifting computation between offline and online phases; product quantization and HNSW, in turn, show that compressed codes and graph topology are not mere implementation details but runtime controls over recall, latency, and memory footprint~\cite{karpukhin2020dpr,khattab2020colbert,jegou2011pq,malkov2020hnsw}. A third family is structured long-horizon memory. Graph- and hierarchy-enhanced approaches such as HippoRAG and RAPTOR push the domain toward memory models that better capture multi-hop and evolving knowledge dependencies, but they also introduce explicit maintenance debt in graph rewiring, abstraction refresh, and stale-structure cleanup~\cite{wang2024hipporag,sarthi2024raptor}.

A fourth family is planner-mediated index governance. Self-RAG makes retrieval invocation itself a runtime control decision rather than a fixed pipeline stage, but that decision only remains meaningful when the runtime also understands whether the underlying memory layer is stale, fragmented, or temporarily expensive to refresh~\cite{asai2024selfrag}. FlowRAG and CANDOR-Bench expose the next step in that control loop: once corpora and ANN structures evolve continuously, the dominant tradeoff is no longer simply retrieve versus do not retrieve, but whether to query a stale index, pay immediate rebuild or compaction cost, or defer maintenance and absorb future quality drift~\cite{zhang2026flowrag,wang2026candor}. Read together with PQ and HNSW, these systems suggest that planner decisions, compression policy, graph maintenance, and rebuild cadence are all part of one governance problem rather than separate offline and online concerns~\cite{jegou2011pq,malkov2020hnsw}.

For systems researchers, the central insight is that retrieval quality is a lifecycle property, not just a model property. Refresh cadence, planner-triggered retrieval policy, index maintenance policy, compression choice, stale-memory eviction, and continuous-ANNS maintenance under churn govern long-term quality-cost balance as strongly as encoder architecture~\cite{asai2024selfrag,wang2026candor,zhang2026flowrag}.

Open gaps include unified treatment of dense, compressed, graph, hierarchical, and planner-mediated memory objects in one control plane, explicit trigger policies for when planners should tolerate staleness versus request rebuilds, and stronger operational metrics that connect refresh, rebuild, and compaction decisions to end-to-end service SLOs rather than isolated retrieval scores.

\subsubsection{Map V: Continual Learning and Retention Governance}

Continual-learning systems contribute a mature language for retention under bounded memory. Constraint-based methods such as EWC treat previously learned parameters as protected state, while GEM and A-GEM reinterpret an episodic buffer plus gradient projection as a runtime control over update interference~\cite{kirkpatrick2017ewc,lopez2017gem,chaudhry2019agem}. Replay-centric methods shift the control surface again: experience replay, MIR, DER, and GDumb all assume that bounded historical memory is inevitable, then ask which samples should be admitted, replayed, distilled, or retained to maximize future utility rather than merely preserve recency~\cite{rolnick2019experience,aljundi2019mir,buzzega2020der,prabhu2020gdumb}. REMIND adds a representation-level variant by showing that feature-space replay and lightweight reconstruction can make retention cheaper without eliminating the underlying state-budget problem~\cite{hayes2020remind}. Read together, these papers formalize retention as a governed state problem in which the runtime must decide what historical signal is worth protecting, how it is stored, and when it should influence future updates.

The strongest mechanisms therefore expose three reusable control surfaces: protected parameter state, episodic buffer selection, and replay-time representation compression or distillation. Their common lesson is that stable adaptation depends on explicit interference control and budget-aware admission policies rather than on unconstrained online updates. This perspective is highly relevant to retrieval and serving systems that increasingly maintain evolving memory under strict resource constraints.

What the literature still under-articulates, but increasingly implies, is a retention-store governance problem. EWC treats the protected set as a scarce budgeted object, GEM and A-GEM make replay eligibility a runtime choice under interference constraints, MIR and GDumb show that admission policy can dominate eventual retained quality, and DER plus REMIND turn representation storage itself into a maintenance surface rather than a free byproduct~\cite{kirkpatrick2017ewc,lopez2017gem,chaudhry2019agem,aljundi2019mir,prabhu2020gdumb,buzzega2020der,hayes2020remind}. Read together, these systems suggest that continual retention should be compared the same way we now compare retrieval indexes or serving caches: not only by accuracy outcomes, but also by how the runtime budgets storage, ranks future utility, compacts stale state, and accounts for maintenance debt over long-lived service horizons.

Open gaps include system-level integration: many continual-learning methods remain model-centric and do not specify operational semantics for distributed retention stores, update fault handling, or cross-service consistency. A second gap is lifecycle accounting: the literature is strong on forgetting metrics but much weaker on maintenance overhead, refresh debt, and retention-store behavior under long-running multi-tenant service conditions.

Figure~\ref{fig:memory-planes} summarizes why these later sections belong together. Serving memory, retrieval memory, and retention stores have different lifetimes, but they all revolve around the same systems question: how a runtime uses shared telemetry and service boundaries to decide which state to admit, refresh, compact, or preserve.
...
MillWheel/Naiad/Dataflow~\cite{akidau2013millwheel,murray2013naiad,akidau2015dataflow} & keyed window and progress state & time/progress semantics & correctness-latency tradeoff & add reconfiguration stress \\
Trill/Differential/\newline Flink/StreamCloud~\cite{chandramouli2014trill,murray2013differential,carbone2015flink,gulisano2012streamcloud} & incremental state and progress metadata & frontier progress and elastic ownership & update latency and operability & connect progress with migration/recovery \\
...
Megaphone/OSM/SEEP~\cite{nasir2019megaphone,fernandez2013osm,fernandez2010seep} & movable state shards & migration and elasticity control & reconfiguration latency & add correctness-envelope taxonomy \\
...
Clockwork/Clipper~\cite{gujarati2020clockwork,crankshaw2017clipper} & serving control-plane state & predictability and \newline orchestration control & tail-latency/SLO isolation & tie to lifecycle admission \\
DistServe/Splitwise/\newline FastGen~\cite{zhong2024distserve,patel2024splitwise,holmes2024fastgen} & disaggregated phase and \newline transfer-queue state & phase split and \newline cluster governance & goodput + tail behavior & unify with tenant-aware admission \\
Nexus/InferLine/INFaaS/\newline Punica/AlpaServe~\cite{shen2019nexus,crankshaw2020inferline,romero2021infaas,chen2024punica,li2023alpaserve} & multiplexed tenant and \newline control-plane state & admission, provisioning, \newline and isolation control & throughput/cost/\newline SLO stability & unify phase and \newline tenant governance \\
...
Self-RAG/FlowRAG/\newline CANDOR~\cite{asai2024selfrag,zhang2026flowrag,wang2026candor} & planner and evolving \newline index state & retrieval gating + \newline rebuild control & long-horizon quality/\newline cost balance & add staleness-trigger policy \\
HippoRAG/\newline FlowRAG~\cite{wang2024hipporag,zhang2026flowrag} & graph-enhanced and evolving \newline retriever memory & dynamic memory \newline evolution control & long-horizon quality \newline and efficiency & add stale-memory debt metrics \\
Continual memory line~\cite{kirkpatrick2017ewc,lopez2017gem,aljundi2019mir,buzzega2020der,hayes2020remind,prabhu2020gdumb,zhou2025ferret,li2026streamfp} & constrained parameters, \newline replay buffers, and \newline compressed retention state & interference control, \newline replay selection, and \newline retention-store governance & forgetting vs \newline adaptation quality & connect with retrieval index retention, \newline maintenance debt, and distributed stores \\
...
Retrieval index evolution & continuous corpus updates + stale data injection + ANN churn & recall@k, answer quality, update cost, query latency, freshness lag, compaction debt & one-shot index build assumption & define refresh-compaction-rebuild schedules \\
